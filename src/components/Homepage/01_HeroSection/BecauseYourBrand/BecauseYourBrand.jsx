...
const BecauseYourBrand = ({
  ...
  return (
    <div className="z-20 left-[20%] text-center" style={{ display: "contents" }}>
      <div
        className={`absolute md:text-[2rem] text-1xl z-10 text-center md:top-[16.5%] top-[18%] w-full ${isMobile ? "opacity-100" : "opacity-0"}`}
        ref={refTxt1}
      >
        {text1}
      </div>
      <div
        className={`absolute left-1/2 -translate-x-1/2 w-[92%] md:w-[96%] text-center mt-[15%] md:top-[18%] top-[22%] md:text-[clamp(4.5rem,10vw,8rem)] text-[3rem] leading-[0.95] z-19 ${anton.className} uppercase whitespace-nowrap ${isMobile ? "opacity-100" : "opacity-0"}`}
        ref={refTxt2}
      >
        {text2}
      </div>
